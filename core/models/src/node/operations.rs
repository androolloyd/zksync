...
use crate::node::{
    pack_date, pack_fee_amount, pack_token_amount, unpack_fee_amount, unpack_token_amount, Close,
    Deposit, FranklinPriorityOp, FullExit, PubKeyHash, Transfer, Withdraw,
};
use crate::params::{
    ACCOUNT_ID_BIT_WIDTH, ADDRESS_WIDTH, AMOUNT_EXPONENT_BIT_WIDTH, AMOUNT_MANTISSA_BIT_WIDTH,
    BALANCE_BIT_WIDTH, CHUNK_BYTES, DATE_EXPONENT_BIT_WIDTH, DATE_MANTISSA_BIT_WIDTH,
    ETH_ADDRESS_BIT_WIDTH, FEE_EXPONENT_BIT_WIDTH, FEE_MANTISSA_BIT_WIDTH, FR_ADDRESS_LEN,
    NEW_PUBKEY_HASH_WIDTH, NONCE_BIT_WIDTH, TOKEN_BIT_WIDTH,
};
...
impl SubscriptionOp {
    ...
    fn get_public_data(&self) -> Vec<u8> {
        ...
        data.extend_from_slice(&pack_token_amount(&self.tx.amount));
        data.extend_from_slice(&pack_fee_amount(&self.tx.fee));
        data.extend_from_slice(&pack_date(&self.tx.startDate));
        data.extend_from_slice(&pack_date(&self.tx.endDate));
        data.extend_from_slice(&pack_fee_amount(&self.tx.fee));
        data.resize(Self::CHUNKS * CHUNK_BYTES, 0x00);
        data
    }
    ...
    pub fn from_public_data(bytes: &[u8]) -> Result<Self, failure::Error> {
            ...
            bytes.len() == Self::CHUNKS * CHUNK_BYTES,
            "Wrong bytes length for subscription pubdata"
        );
        ...
        let start_offset =
            amount_offset + (AMOUNT_EXPONENT_BIT_WIDTH + AMOUNT_MANTISSA_BIT_WIDTH) / 8;
        let end_offset = start_offset + (DATE_EXPONENT_BIT_WIDTH + DATE_MANTISSA_BIT_WIDTH) / 8;
        let fee_offset = end_offset + (DATE_EXPONENT_BIT_WIDTH + DATE_MANTISSA_BIT_WIDTH) / 8;
